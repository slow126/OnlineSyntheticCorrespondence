...
def extract_flow_vectors_at_keypoints_batch(
    ...
    Returns:
        pred_corr_flows: [N_corr, 4] flow vectors [x, y, dx, dy] for correct predictions
        pred_miss_flows: [N_miss, 4] flow vectors [x, y, dx, dy] for incorrect predictions
        gt_flows: [N_total, 4] flow vectors [x, y, dx, dy] for ground truth at all keypoints
    """
    ...
    for b in range(batch_size):
        ...
        # Get keypoints for this sample (only valid ones)
        kps = trg_kps[b, :, :npt]  # [2, npt]
        kps_x = kps[0].long()  # [npt]
        kps_y = kps[1].long()  # [npt]
        
        # Clamp to valid image bounds
        _, H, W = pred_flow.shape[1:]
        kps_x = torch.clamp(kps_x, 0, W - 1)
        kps_y = torch.clamp(kps_y, 0, H - 1)
        
        # Extract flows at keypoint locations
        # pred_flow[b]: [2, H, W], kps_y: [npt], kps_x: [npt]
        pred_dx = pred_flow[b, 0, kps_y, kps_x]  # [npt]
        pred_dy = pred_flow[b, 1, kps_y, kps_x]  # [npt]
        ...
        if len(correct_ids[b]) > 0:
            correct_mask[correct_ids[b]] = True
