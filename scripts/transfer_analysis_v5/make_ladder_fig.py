...
metric -- raw JOINT mean-NN distance d_{T->S} (coverage) and d_{S->T} (off-target),
...
KITTI-2012 and KITTI-2015 are plotted as separate lines. Coverage d_{T->S} dips to a
minimum at the matched magnitude (best coverage) and rises on either side; off-target
d_{S->T} stays flat (<=1% swing). Right two panels: peak-PCK heatmaps (inverted-U).
...
# Raw mean-NN (NOT qnorm): d_{T->S}=mean_nn_eval_to_train (coverage),
# d_{S->T}=mean_nn_train_to_eval (off-target). Per benchmark, averaged over apps.
# NOTE: analysis/coverage_v2_flow_ladder.csv was regressed by a Jun-25 m-rung vector
# re-render (coverage went monotonic instead of dipping at the match). Use the
# recovered pre-regression data, which reproduces the dip (matches protoA5/A7).
cv = pd.read_csv("analysis/coverage_v2_flow_ladder_predip_GOOD.csv")
...
# coverage d_{T->S} on the left axis (dips at the match); off-target d_{S->T} on a
...
ax.set_ylabel(r"Coverage  $d_{T\to S}$  (mean-NN, lower=better)", color=COV15)
ax2.set_ylabel(r"Off-target  $d_{S\to T}$  (mean-NN)", color=OFF15)
...
ax_a.set_ylabel(r"Coverage  $d_{T\to S}$  (mean-NN, lower=better)", color=COV15)
ax_a2.set_ylabel(r"Off-target  $d_{S\to T}$  (mean-NN)", color=OFF15)
