...
SNAP_DIR = Path("/mnt/nvme_1tb_a/snapshots/transfer_grid_glunet")
...
KUBRIC_SOURCES = {
    ...
    # FlyingThings grounded-search best (search_v4 trial_0040, 2500 scenes).
    # Run with --only ft_t0040 --min-scenes 2400.
    "ft_t0040":                     "ft_search_v4_t0040_gso_hq_2500",
}
SYNTH_SOURCES = {"synthetic_fractal_trial76": SYNTH_TEMPLATE}

VARIANTS = {"TT": (True, True), "TF": (True, False), "FF": (False, False)}
...
GRID: list[tuple[str, str]] = (
    ...
    + [(s, "TT") for s in PRETRAINED_TT_SOURCES]
    + [("synthetic_fractal_trial76", "TT")]
    + [("ft_t0040", "TF")]   # trial_0040: pretrained backbone, fine-tuned (not frozen)
)
...
def main() -> None:
    ...
    ap.add_argument("--min-scenes", type=int, default=MIN_SCENES,
                    help="readiness threshold for a source's scene count "
                         f"(default {MIN_SCENES}; lower for sub-5000 test datasets).")
    args = ap.parse_args()
