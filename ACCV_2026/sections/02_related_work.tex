...
FlyingChairs/FlyingThings \cite{dosovitskiy2015Flownet-FlyingChairsDataset,mayer2016FlyingThingsDataset}, Sintel
...
our finding makes the match \emph{directional}. Coverage of the target's motion is
the half that predicts transfer, and a pretrained backbone is what lets a model use
it. A large, active
...
what \emph{motion} to generate, which is why the same rule guides two structurally
different generators (Sect.~\ref{sec:interv}).
Procedural abstraction \cite{Baradad2021LookingAtNoise,Kataoka2020Pre-trainingImages,Anderson2022ImprovingFractal,white2009mandelbulb}
...
like ours, a fit-free dataset distance, but symmetric: its label-to-label optimal
transport assumes a discrete class space, which dense correspondence lacks, so it
does not apply here. Our sliced-$W_2$ baseline needs no such class space, so the OT
family \emph{is} represented among our symmetric comparisons. None of these methods address dense
...
\emph{covers} a given target's, a directed quantity rather than the symmetric
distribution match those methods optimize. On the theory side, asymmetric transfer
exponents \cite{hanneke2019value} and covariate-shift support conditions \cite{bendavid2010theory} anticipate
the headline (coverage of the target's support predicts transfer), but not how
cheaply it can be measured (a crude motion histogram, no training) or that the
off-target half carries almost none of the signal. Our dataset-size controls
(Sect.~\ref{sec:law}) rule out a trivial ``more data'' explanation.
...
competing hypothesis that our controls reject for within-context dataset
ranking.
