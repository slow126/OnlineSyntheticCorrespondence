...
\cite{greff2021kubric} plus our SDF-fractal generator, an asset-free procedural
...
descriptor actually produces (\emph{geometry-only}: each candidate costs only a
...
of the geometry-only \emph{tuned} source (the KITTI-recovered generator,
...
semantic benchmarks). The full grid, including additional backbone
configurations, is reported in the supplement.}
...
built without a single real-target label beats a generic synthetic dataset.

\begin{table}[t]
\centering
\caption{\textbf{The coverage objective is renderer-agnostic (KITTI).} Peak \pck{}
of the SDF-fractal generator's coverage-tuned vs.\ default source, under a
pretrained backbone. \textbf{Bold} $=$ tuned improves on default (five of six
cells).}
\label{tab:interv_fractal}
\small
\input{tables/tab_interv_fractal}
\end{table}

\noindent\textbf{The objective is renderer-agnostic.}
The same geometry-only coverage search, run on our SDF-fractal generator (which
shares no rendering code or assets with Kubric), produces a KITTI source whose
tuned motion matches or improves on the generator's default in five of six backbone
cells and lands on par with the best tuned Kubric source
(Table~\ref{tab:interv_fractal}). Two structurally unrelated generators, tuned to
the same target by the same motion-coverage objective, arrive at the same place---so
the rule is a property of the motion, not of any one renderer.
...
(Fig.~\ref{fig:ladder}), confirming motion, not realism or object content, as the
