...
anchor ($|f|{\approx}4$); training on it beats MOVi-F across all configurations
...
all $14$ KITTI cells. On the semantic \textsc{tss} benchmark (right; pretrained
...
transfer; even for the from-scratch CATs++ config (an out-of-distribution probe, supp.)
...
in \emph{all fourteen} KITTI settings, with gains of $+1.8$ to $+22.5$ \pck{}
across architectures and backbone configurations (Table~\ref{tab:interv}). The
CATs++-scratch wins come from prior-matching rather than learned
features---a source whose motion marginal sits closer to the target transfers
better even without a trained encoder. A target-specific source built without a
single real-target label beats a generic synthetic dataset.
