...
float sineFieldDE(vec3 p) {
    ...
    vec3 planeNormal = normalize(rotationMatrix * vec3(0.0, 1.0, 0.0));
    vec3 planePoint = vec3(0.0, -2.0, 0.0);
    
    // Transform point into plane's local space
    vec3 localP = p - planePoint;
    float distToPlane = dot(localP, planeNormal);
    
    // Get sine field value
    float fieldValue = amp * (sin(freq * p.x) + sin(freq * p.y) + sin(freq * p.z));
    
    return distToPlane - fieldValue;
}

vec3 calcNormal(vec3 p) {
    vec2 e = vec2(DEL, 0.0);
    return normalize(vec3(
        sineFieldDE(p + e.xyy) - sineFieldDE(p - e.xyy),
        sineFieldDE(p + e.yxy) - sineFieldDE(p - e.yxy),
        sineFieldDE(p + e.yyx) - sineFieldDE(p - e.yyx)
    ));
}
...
vec4 render(vec3 ro, vec3 rd, vec3 light, float epsilon) {
    const vec4 backgroundColor = vec4(0.6, 0.8, 1.0, 1.0);
