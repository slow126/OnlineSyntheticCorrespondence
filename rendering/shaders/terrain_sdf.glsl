...
uniform vec4      iJuliaC         = vec4(1.0, 1.0, 1.0, 1.0);

// Pseudo-random numbers
float iHashSeed = 43758.5453123; // Harcoded seed
...
float valueNoise(vec2 p) {
    vec2 pi = floor(p); // Grid cell coordinates
    vec2 pf = fract(p); // Local coordinates within cell

    // Random values at the corners of the cell
    float v00 = hash2(pi);
    float v01 = hash2(pi + vec2(0.0, 1.0));
    float v10 = hash2(pi + vec2(1.0, 0.0));
    float v11 = hash2(pi + vec2(1.0, 1.0));

    // Interpolation weights
    vec2 w = pf * pf * (3.0 - 2.0 * pf);

    // Bilinear interpolation
    float x1 = mix(v00, v10, w.x);
    float x2 = mix(v01, v11, w.x);
...
float fbm(vec2 p) {
    ...
    for (int i = 0; i < 4; i++) {
        addon = 0.;
        addon += amplitude * valueNoise(p * frequency);
        addon += amplitude * valueNoise((p + vec2(79.5, 15.7)) * frequency);
        addon += amplitude * valueNoise((p + vec2(914.4, 831.5)) * frequency);
        addon/=3.;
        value += addon;
        p *= 2.0;
    ...
    float height_increase_factor = 0.0001;
    return value + .02 - (height_increase_factor * (p.x*p.x + p.y*p.y));
}

float terrainSDF(vec3 p) {
    vec2 tp = vec2(p.x, p.z);
    float height = fbm(tp * 0.1) * -10.0; // Scale the noise and height
    return p.y - height - 0.0;
}
...
float intersect(inout vec3 rO, inout vec3 rD, float epsilon) {
    ...
    // Maximum step multiplier when far away
    float MAX_STEP_MULT = 5.0;
    
    // Start with a smaller number of iterations
    for(int i = 0; i < 80; i++) {
        dist = terrainSDF(rO);
        
        // Adaptive step size: larger when far, smaller when close
        // clamp ensures we never step over boundaries
        float stepMult = clamp(dist / epsilon, 1.0, MAX_STEP_MULT);
        
        // Take larger steps when further away
        t += dist * stepMult;
        rO += rD * dist * stepMult;
        
        // Early termination conditions
        // Check for hit or max distance
        if(dist < epsilon || t > BOUNDING_RADIUS) 
            break;
    }
    return dist;
}

//TODO: We can get rid of the lighting model since texture is computed after the fact
...
void main() {
    iHashSeed = iHashSeed*(1.+iJuliaC.x)/2.; // External Random Source
    ...
    if (out_coord == vec3(0.0)) {
        out_depth = BOUNDING_RADIUS + 10.0;
        out_object_id = -1;
