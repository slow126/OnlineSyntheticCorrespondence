...
float v000 = hash3(pi);
float v001 = hash3(pi + vec3(0.0, 0.0, 1.0));
float v010 = hash3(pi + vec3(0.0, 1.0, 0.0));
float v011 = hash3(pi + vec3(0.0, 1.0, 1.0));
float v100 = hash3(pi + vec3(1.0, 0.0, 0.0));
float v101 = hash3(pi + vec3(1.0, 0.0, 1.0));
float v110 = hash3(pi + vec3(1.0, 1.0, 0.0));
float v111 = hash3(pi + vec3(1.0, 1.0, 1.0));
...
//Deformed sphere
float objSDF(vec3 p, vec3 center, float r) {
    // Base sphere distance
    r = r-0.2;
    float baseDistance = length(p - center) - r;

    // Apply random deformation using Perlin noise
    float deformation = valueNoise3d(p * 2.0 + vec3(iHashSeed)) * 0.6;

    // Add the deformation to the sphere distance
    return baseDistance + deformation;
}

vec3 calcNormal(vec3 p) {
    vec2 e = vec2(DEL, 0.0);
    return normalize(vec3(
        objSDF(p + e.xyy, vec3(0.0), 1.0) - objSDF(p - e.xyy, vec3(0.0), 1.0),
        objSDF(p + e.yxy, vec3(0.0), 1.0) - objSDF(p - e.yxy, vec3(0.0), 1.0),
        objSDF(p + e.yyx, vec3(0.0), 1.0) - objSDF(p - e.yyx, vec3(0.0), 1.0)
    ));
}
...
void main() {
    ...
    for (int i = 0; i < AA; i++)
        for (int j = 0; j < AA; j++) {
            ...
            color += render(camera, rd, lightSource, 9e-5);
        }
    color /= float(AA * AA);
