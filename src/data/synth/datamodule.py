...
class SynthCorrespondenceDatamodule(BaseDatamodule):
    def __init__(
        self,
        root: Union[str, Path],
        imgs_per_obj: Optional[int] = None,
        num_train: Optional[int] = None,
        num_val: Optional[int] = 10,
        img_res: Union[int, tuple, list] = 256,
        flow_res: Union[int, tuple, list] = 64,
        label_type: Literal['flow', 'points'] = 'flow',
        flow_map: str = 'relative',
        ...
        self.imgs_per_obj = imgs_per_obj
        ...
        self.relative_flow = flow_map == 'relative'

        if label_type not in ('flow', 'points'):
            raise ValueError(f'label_type must be one of ("flow", "points") but got "{label_type}"')

        if label_type == 'points' and num_points is None:
            raise ValueError(f'Must provide num_points if label_type == "points"')

        super().__init__(root, batch_size, num_workers, shuffle, copy_data_local=copy_data_local)

    def post_init(self):
        self.dataset_class = hdf5_dataset.Hdf5CorrespondenceDataset

    def prepare_data(self):
        if self.copy_data_local is not None:
            self.pattern = '*.{hdf5,json}'
        super().prepare_data()

    def get_transforms(self):
        r = [int(8 / 7 * s) for s in self.img_res]
        as_int = self.label_type == 'flow'
        train = transforms.Compose([
            transforms.ToTensor(),
            transforms.Resize(r, as_int=as_int),
            transforms.RandomCrop(self.img_res),
            transforms.ColorJitter(0.2, 0.2, 0.2),
            transforms.ScalePoints(self.flow_res, as_int=as_int),
            transforms.Normalize(0.5, 0.5),
        ])
        val = transforms.Compose([
            transforms.ToTensor(),
            transforms.Resize(self.img_res, as_int=as_int),
            transforms.ScalePoints(self.flow_res, as_int=as_int),
            transforms.Normalize(0.5, 0.5),
        ])
        return train, val
    ...
    def setup(self, stage):
        transform, transform_val = self.get_transforms()
        offset = self.num_val or 0
        self.train_data = self.dataset_class(
            self.root, transform=transform, obj_limit=self.num_train, imgs_per_obj=self.imgs_per_obj, offset=offset)
        self.val_data = self.dataset_class(
            self.root, transform=transform_val, obj_limit=offset, imgs_per_obj=self.imgs_per_obj)

    def collate(self, batch):
        imgs = [x[:2] for x in batch]
        pts = [x[2] for x in batch]
        imgs = torch.utils.data.default_collate(imgs)
        if self.label_type == 'flow':
            flow = corr2flow(pts, self.flow_res, self.relative_flow)
            return {'src_img': imgs[0], 'trg_img': imgs[1], 'flow': flow}
        elif self.label_type == 'points':
            points = sample_kps(pts, self.num_points)
            return {'src_img': imgs[0], 'trg_img': imgs[1], 'points': points}


class SyntheticEvaluationDatamodule(BaseDatamodule):
    def __init__(
        self,
        root: Union[str, Path],
        img_res: Union[int, tuple, list] = 256,
        normalize: Union[bool, str, list, tuple] = 'imagenet',
        batch_size: int = 32,
        num_workers: int = 8,
        copy_data_local: Optional[str] = None,
    ):
        self.dataset_class = hdf5_dataset.HDF5ImageGeometryDataset
    ...
    def setup(self, stage):
        if stage == 'fit':
            raise RuntimeError('SyntheticEvaluationDatamodule should only be used for evaluation')
        
        self.val_data = self.dataset_class(self.root, self.img_res)
    ...
    def on_after_batch_transfer(self, batch: Any, dataloader_idx: int=0):
        ...
        for i, k in enumerate(('src', 'trg')):
            img = batch[i]['image'].moveaxis(-1, 1).mul(1 / 255)
...
class ComponentsDatamodule(BaseComponentsDatamodule):
    '''Datamodule for loading paired images. Handles rendering images based
    on geometry and normals.
    '''
    def __init__(
        self,
        root: Union[str, Path],
        val_root: Optional[Union[str, Path]] = None,
        ...
        texture_alpha: Union[float, tuple] = 3.5,
        consistent_texture: bool = True,
        background_flow: float = 0.0,
        random_flip: Optional[float] = None,
        random_swap: bool = True,
        subsample_flow: Optional[float] = None,
        ...
        super().__init__(
            ...
            img_res = img_res,
            consistent_texture = consistent_texture,
    ...
    def post_init(self):
        self.dataset_class = hdf5_dataset.Hdf5CorrespondenceComponentsDataset
    ...
    def setup(self, stage='fit'):
        ...
        if self.val_root is None:
            self.val_root = self.root.joinpath('eval')
        self.val_data = self.dataset_class(
            root = self.val_root, 
